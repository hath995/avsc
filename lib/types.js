...
/*
 Given an array of buffers, write them to a tap
*/
ArrayType.prototype._pack = function (tap, val) {
  if (!Array.isArray(val)) {
    throwInvalidError(val, this);
  }
...
/*
 * Replace an element in an ArrayType buffer with a new element
 * @param tap {Tap} A tap positioned at the beginning of an array block.
 * @param val {itemsType} a value to replace an element
 * @param index {number}
 * @returns {number} new length of the encoded array in bytes
 * 8 items, 3, 2, 1, 1, 1, 0 //block sizes
 * update block 4
 */
ArrayType.prototype._merge = function(tap, val, index) {
  var arrayStart = tap.pos;
  this._skip(tap);
  var arrayEnd = tap.pos;
  tap.pos = arrayStart;
  var blockSize = readArraySize(tap)
  var i = 0;
  //skip blocks with item indices less than index
  while(blockSize + i <= index) {
    if(blockSize == 0) {
      throw new Error("Index " + index + " out of Array length");
    }
    for(var k = 0;  k < blockSize; k++) {
      this.itemsType._skip(tap)
    }
    i += blockSize;
    blockSize = readArraySize(tap);
  }
  //skip over any remaining elements within the block
  ...
  this.itemsType._skip(tap);
  var end = tap.pos;
  //copy entire buffer from end onward in case of data beyond the array
  var copyBuffer = utils.newBuffer(tap.buf.length-end);
  tap.buf.copy(copyBuffer, 0, end);
  tap.pos = pos;
  this.itemsType._write(tap, val);

  //if the data written + the rest of the array is larger than the tap buffer embiggen
  //or maybe throw an error?
  if(tap.pos + arrayEnd-end > tap.buf.length) {
    var overflowBuffer = utils.newBuffer(tap.pos + arrayEnd-end);
    tap.buf.copy(overflowBuffer, 0, 0, tap.pos);
    tap.buf = overflowBuffer;
  }
  copyBuffer.copy(tap.buf, tap.pos, 0);
  return tap.pos + arrayEnd-end;
}
